...
type Distance = 'EUCLIDEAN' | 'MANHATTAN' | 'HAVERSINE';

interface IjDBSCAN {
  data(d: IDataPointMovement[]): IjDBSCAN;

  eps(e: number): IjDBSCAN;

  minPts(p: number): IjDBSCAN;

  distance(fn: Distance): IjDBSCAN;

  timeDistance(fn): IjDBSCAN;

  timeEps(e: number): IjDBSCAN;
}

const jDBSCAN = function(): IjDBSCAN {
  // local instance vars.
  let eps: number;
  let time_eps: number;
  let minPts: number;
  let data: IDataPointMovement[] = [];
  let clusters = [];
  let status = [];
  let graph = [];
  ...
  // distance functions
  function timestamp_distance(point1: IDataPointMovement, point2: IDataPointMovement) {
    return Math.abs(point2.timestamp - point1.timestamp);
  }

  function euclidean_distance(point1: IDataPointMovement, point2: IDataPointMovement) {
    return Math.sqrt(Math.pow(point2.X - point1.X, 2) + Math.pow(point2.Y - point1.Y, 2));
  }

  function manhattan_distance(point1: IDataPointMovement, point2: IDataPointMovement) {
    return Math.abs(point2.X - point1.X) + Math.abs(point2.Y - point1.Y);
  }

  function haversine_distance(point1: IDataPointMovement, point2: IDataPointMovement) {
    const R: number = 6371;
  ...
  // getters and setters
  dbscan.data = function(d: IDataPointMovement[]) {
  ...
  dbscan.timeEps = function(e: number) {
    if (arguments.length === 0) {
      return time_eps;
    }
  ...
  dbscan.minPts = function(p: number) {
    if (arguments.length === 0) {
      return minPts;
    }
  ...
  dbscan.distance = function(fn: Distance) {
    if (arguments.length === 1) {
      if (typeof fn === 'string') {
        switch (fn) {
          case 'HAVERSINE':
