...
const dbScanWorker = {
  dbscan: (data:IDataPointMovement[], eps:number,minPts:number,timeEps:number, useTime:boolean) => {
    let result: Function;
    if (useTime) {
      result = jDBSCAN()
        //@ts-ignore
    ...
    return {dp:result(), clusters:result.getClusters()};
  }
};
...
export type  DbScanWorker = typeof dbScanWorker
